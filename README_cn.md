...
- **功能**: 从指定URL获取元数据（精确模式）
- **使用场景**: 知道具体图库URL时使用
- **工作流程**: 将URL粘贴到题名字段，启用此选项，然后下载元数据
- **默认值**: 关闭
- **注意**: 不再弹出对话框 - URL必须粘贴到题名字段中
...
#### 操作步骤 (新方法 - v3.0.0+)
1. 将本子URL粘贴到**题名**字段中 (例如: `https://e-hentai.org/g/1234567/abcdef123456/`)
2. 在插件设置中启用"Accurate label mode"
3. 下载元数据 - 插件将从指定URL获取数据

#### 重要变更
- **v2.x**: 弹出对话框输入URL
- **v3.0.0+**: 无对话框 - URL必须粘贴到题名字段
- **原因**: Calibre工作进程无法显示GUI对话框
...
- 精确模式变更：从弹出对话框改为URL粘贴到题名字段的方式（修复工作进程崩溃问题）
